...
public class Parallaxer : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        for (int i = 0; i < layers.Length; i++)
        {
            vect2.x = (cameraTrfm.position.x - referencePoint.x) * calculatedRates[i].x;
            vect2.y = (cameraTrfm.position.y - referencePoint.y) * calculatedRates[i].y;
